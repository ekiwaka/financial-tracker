...
import copy
import calendar
import app.views.dashboard as view_dashboard
import app.views.expenses as view_expenses
import app.views.budgets as view_budgets
import app.views.categories as view_categories
import app.views.reports as view_reports
import app.views.account as view_account
...
def create_app():
    ...
    def index():
        ...
        if request.method == "GET":
            ...
            categories = view_categories.getSpendCategories(session["user_id"])

            # Get the users payers (for quick expense modal)
            payers = view_account.getPayers(session["user_id"])

            # Get todays date (for quick expense modal)
            date = datetime.today().strftime('%Y-%m-%d')

            # Get the users income
            income = view_account.getIncome(session["user_id"])

            # Get current years total expenses for the user
            expenses_year = view_dashboard.getTotalSpend_Year(session["user_id"])

            # Get current months total expenses for the user
            expenses_month = view_dashboard.getTotalSpend_Month(
                session["user_id"])

            # Get current week total expenses for the user
            expenses_week = view_dashboard.getTotalSpend_Week(session["user_id"])

            # Get last 5 expenses for the user
            expenses_last5 = view_dashboard.getLastFiveExpenses(
                session["user_id"])

            # Get every budgets spent/remaining for the user
            budgets = view_dashboard.getBudgets(session["user_id"])

            # Get weekly spending for the user
            weeks = view_dashboard.getLastFourWeekNames()
            spending_week = view_dashboard.getWeeklySpending(
                weeks, session["user_id"])

            # Get monthly spending for the user (for the current year)
            spending_month = view_dashboard.getMonthlySpending(
                session["user_id"])

            # Get spending trends for the user
            spending_trends = view_dashboard.getSpendingTrends(
                session["user_id"])

            # Get payer spending for the user
            payersChart = view_reports.generatePayersReport(session["user_id"])
        ...
        # User reached route via POST
            ...
            expenses = view_expenses.addExpenses(formData, session["user_id"])
    ...
    def expensehistory():
        ...
        if request.method == "GET":
            ...
            history = view_expenses.getHistory(session["user_id"])
        ...
        # User reached route via POST
            ...
            oldExpense = view_expenses.getExpense(
            ...
            if userHasSelected_deleteExpense == True:
                ...
                deleted = view_expenses.deleteExpense(
                ...
                history = view_expenses.getHistory(session["user_id"])
                categories = view_categories.getSpendCategories(
                    session["user_id"])
                payers = view_account.getPayers(session["user_id"])
            ...
            # Update the existing expense record
                ...
                expensed = view_expenses.updateExpense(
    ...
    def budgets(year=None):
        ...
        if request.method == "GET":
            ...
            budgets = view_budgets.getBudgets(session["user_id"])

            # Get the users total budgeted amount
            budgeted = view_budgets.getTotalBudgetedByYear(
        ...
        # User reached route via POST
            ...
            # Delete the budget
            deletedBudgetName = view_budgets.deleteBudget(
    ...
    def createbudget():
        ...
        if request.method == "POST":
            ...
            budgetDict = view_budgets.generateBudgetFromForm(formData)
            ...
            if "apology" in budgetDict:
                ...
                budget = view_budgets.createBudget(
            ...
            # Get the users total budgeted amount
            budgeted = view_budgets.getTotalBudgetedByYear(session["user_id"])
    ...
    def updatebudget(urlvar_budgetname):
        ...
        if request.method == "POST":
            ...
            if "apology" in budgetDict:
                ...
                budget = view_budgets.updateBudget(
        ...
        # User reached route via GET
            ...
            budgetID = view_budgets.getBudgetID(
                urlvar_budgetname, session["user_id"])
            if budgetID is None:
                return apology("'" + urlvar_budgetname + "' budget does not exist")
            else:
                budget = view_budgets.getBudgetByID(budgetID, session["user_id"])
            ...
            budget = view_budgets.getUpdatableBudget(budget, session["user_id"])
    ...
    def categories():
        ...
        if request.method == "POST":
            ...
            if userHasSelected_newCategory:
                ...
                categoryCount = len(
                    view_categories.getSpendCategories(session["user_id"]))
                if categoryCount >= 30:
                ...
                categoryID = view_categories.getCategoryID(newCategoryName)
                ...
                if categoryID:
                    ...
                    existingID = view_categories.getCategoryID(
                    ...
                    # Add the category to the users account
                        ...
                        view_categories.addCategory_User(
                ...
                # Category does not exist in the DB already - create a new category and then add it to the users account
                    ...
                    newCategoryID = view_categories.addCategory_DB(
                        newCategoryName)
            ...
            if userHasSelected_renameCategory:
                ...
                oldCategoryID = view_categories.getCategoryID(oldCategoryName)
                ...
                # Check to see if the *new* name already exists in the database (None == does not exist)
                newCategoryID = view_categories.getCategoryID(newCategoryName)
                ...
                if newCategoryID:
                    ...
                    newCategoryNameFromDB = view_categories.getSpendCategoryName(
                        newCategoryID)

                    # Rename the category
                    view_categories.renameCategory(
            ...
            if userHasSelected_deleteCategory:
                ...
                categoryID = view_categories.getCategoryID(deleteName)
                ...
                # Delete the category
                view_categories.deleteCategory(categoryID, session["user_id"])
        ...
        # User reached route via GET
            ...
            categoryBudgets = view_categories.getBudgetsSpendCategories(
                session["user_id"])

            # Generate a single data structure for storing all categories and their associated budgets
            categoriesWithBudgets = view_categories.generateSpendCategoriesWithBudgets(
    ...
    def budgetsreport(year=None):
        ...
        budgets = view_reports.generateBudgetsReport(session["user_id"], year)
    ...
    def monthlyreport(year=None):
        ...
        monthlySpending = view_reports.generateMonthlyReport(
            session["user_id"], year)
    ...
    def spendingreport(year=None):
        ...
        spendingReport = view_reports.generateSpendingTrendsReport(
            session["user_id"], year)
    ...
    def payersreport(year=None):
        ...
        payersReport = view_reports.generatePayersReport(
            session["user_id"], year)
    ...
    def updateaccount():
        ...
        if request.method == "POST":
            ...
            if userHasSelected_updateIncome:
                ...
                updatedIncome = view_account.updateIncome(
            ...
            if userHasSelected_addPayer:
                ...
                # Add the payer
                newPayer = view_account.addPayer(newName, session["user_id"])
            ...
            if userHasSelected_renamePayer:
                ...
                # Rename the payer
                renamedPayer = view_account.renamePayer(
            ...
            if userHasSelected_deletePayer:
                ...
                # Delete the payer
                deletedPayer = view_account.deletePayer(name, session["user_id"])
            ...
            if userHasSelected_updatePassword:
                ...
                updatedPassword = view_account.updatePassword(request.form.get(
            ...
            user = view_account.getAllUserInfo(session["user_id"])
